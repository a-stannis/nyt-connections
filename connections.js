let buttonsEl = Array.from(document.querySelectorAll('.button'));
      ...
      let series1= ['john', 'paul', 'ringo', 'george'];
      let series2=['leonardo', 'michelangelo', 'donatello', 'raphael'];
      let series3= ['war', 'famine', 'death', 'pestilence'];
      let series4= ['regina', 'karen', 'gretchen', 'cady'];
      let series1topic= ['The beatles']
      let series2topic= ['teenage mutant ninga tutrles']
      let series3topic= ['horseman of the apocalypse']
      let series4topic= ['mean girls']
      let series1buttons=[];
      let series2buttons=[];
      let series3buttons=[];
      let series4buttons=[];
      ...
      function pickteams(){
        ...
        buttonsforsorting.splice(number, 1);
          ...
          } else{series4buttons.push(button);
            originalSeries1Buttons.push(button);}
      }
...
//event listeners
  ...
  if (!button.classList.contains('win1') && !button.classList.contains('win2') && 
  !button.classList.contains('win3') && !button.classList.contains('win4')) {
...
//pressing buttons
          ...
          function press(button) {
            if (Array.from(button.classList).includes('win1') || Array.from(button.classList).includes('win2') ||
             Array.from(button.classList).includes('win3') || Array.from(button.classList).includes('win4')) {
            return;
          }
...
function solverest(){
  ...
  for (const p of quadsPressed) {
    ...
    const isSame = quadButtonIDs.every((buttonID, index) => buttonID === pButtonIDs[index]);
    if (isSame) {
      for (element of pressed) {
        element.classList.remove('pressed');
      }
...
const buttonsToReplace = Array.from(document.querySelectorAll(`#row${row} .button`));
...
for (const b of buttonsToReplace) {
  ...
  if (matchingWinButton) {
    const availablePair = pairlist.find((pair) => pair.isSame === false && pair.done === false);
    if (availablePair) {
